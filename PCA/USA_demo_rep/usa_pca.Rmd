...
1. homelessness - homelessness per 100000 inhabitants in respective state
...
4. suicide_rate - number of suicide cases by inhabitants in respective state
5. rape_rate - number of rape cases by inhabitants in respective state
...
```{r}
merged_df <- merged_df %>%
  select(c("suicide_rate","rape_rate","guns_ownership","homelessness","homicide_mortality_rate",
           "governor"))
```
...
* if homelessness increases, it has a slight positive effect on PC1, PC2 strongly increases
...
2. Variables that are negatively correlated: homelessness and homicide_mortality_rate
